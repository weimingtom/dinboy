...
package cordy.tools{
	...
	public class filters {
		...
		public static function noiseFilter(source:BitmapData,degree:Number=128):BitmapData {
			//degree 0-255
			var noise:uint,color:uint,r:uint,g:uint,b:uint;
		...
		public static function sketchFilter(source:BitmapData,threshold:Number=30):BitmapData {
			...
			returnBitmapData=new BitmapData(sourceBitmap.width,sourceBitmap.height,true, 0x00FFFFFF);
			returnBitmapData.draw(sourceBitmap);
			var color:uint,gray1:uint,gray2:uint;
		...
		public static function lightingFilter(source:BitmapData,power:Number=128,posx:Number=0.5,posy:Number=0.5,ar:Number=0):BitmapData {
			//power 0-255
			var midx:int=int(source.width*posx);
			var midy:int=int(source.height*posy);
			if (ar==0) {
				ar=Math.sqrt(midx*midx+midy*midy);
			}
			if (ar==0) {
				ar=Math.sqrt(source.width*source.width/4+source.height*source.height/4);
			}
		...
		public static function mosaicFilter(source:BitmapData,block:Number=6):BitmapData {
			//block 1-32
			returnBitmapData=source.clone();
			var sumr:uint,sumg:uint,sumb:uint,product:int,color:uint,r:uint,g:uint,b:uint,br:uint,bg:uint,bb:uint;
